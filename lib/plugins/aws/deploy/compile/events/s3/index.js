...
class AwsCompileS3Events {
  ...
  compileS3Events() {
    ...
    _.forEach(bucketsLambdaConfigurations, (bucketLambdaConfiguration, bucketName) => {
      ...
      if (definedResource) {
        const properties = definedResource.Properties;
        const bucketName = properties.BucketName;


        if (!properties.NotificationConfiguration) {
          properties.NotificationConfiguration = {};
        }
        if (!properties.NotificationConfiguration.LambdaConfigurations) {
          properties.NotificationConfiguration.LambdaConfigurations = [];
        }

        _.each(bucketLambdaConfiguration, function(event) {
          properties.NotificationConfiguration.LambdaConfigurations.push(event);
        });
        console.log(bucketName, 'already defined, update', properties);
      // Create new bucket
      } else {
        ...
        console.log('create bucket', bucketTemplate);
